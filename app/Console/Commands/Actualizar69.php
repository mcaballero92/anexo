...
class Actualizar69 extends Command {
	 ...
	 * @var string
	 */
	protected $signature = 'anexo:actualizar';
		...
		*/
		...
		if (Schema::hasTable('69_Clone')) {
			...
			if ($Nuevo == $Clone && $Nuevo != 0) {
				...
				Mail::send('layouts.anexo', ['antes' => $Clone, 'despues' => $Nuevo], function ($message) use ($email_data) {
		...
		Mail::send('layouts.anexo', ['antes' => $Clone, 'despues' => $Nuevo], function ($message) use ($email_data, $dump_sql) {
			$message->from('cmiguel@advans.mx', 'Laravel')
				->to($email_data['recipient'])
				->subject($email_data['subject'])
				->attach($dump_sql);
